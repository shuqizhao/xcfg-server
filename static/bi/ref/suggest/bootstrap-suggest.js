    ...
    var methods = {
        init: function(opts){
            ...
            return self.each(function(){
                var $input = $(this),
                    $dropdownMenu = $input.parents(".input-group:eq(0)").find("ul.dropdown-suggest");
                //验证输入框对象是否符合条件
                if(checkInput(this) === false){
                    console.warn('not a dropdown-suggest');
                    return;
                }
            ...
            */
            function checkInput(target){
                var $input = $(target),
                    $dropdownMenu = $input.parent(".input-group").find("ul.dropdown-suggest"),
            ...
            function refreshDropMenu($input, data, opts) {
                var $dropdownMenu = $input.parent().find("ul.dropdown-suggest"),
                len, i, j, index = 0,
        ...
        show: function(){
            ...
            if (data && data.options) {
                this.parent().find("ul.dropdown-suggest").show();
            }
            return this;
        },
        hide: function(){
            var data = this.data('bsSuggest');
            if (data && data.options) {
                this.parent().find("ul.dropdown-suggest").css("display","");
            }
            return this;
        },
        disable: function (e) {
